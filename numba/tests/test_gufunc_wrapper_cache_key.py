...
class TestGufWrapperCacheKey(TestCase):
    ...
    def _key_for(self, symbol):
        cache = _KernelKeyedGufWrapperCache(_plain_kernel, symbol)
    ...
    def test_symbol_is_appended_to_base_key(self):
        # The kernel symbol extends the base key rather than replacing it, so
        # signature/target/bytecode discrimination is preserved.
        key = self._key_for('_ZN1fB2v1E')
        self.assertEqual(key[-1], '_ZN1fB2v1E')
        base = GufWrapperCache(_plain_kernel)
        self.assertEqual(key[:-1],
                         base._index_key('float64(float64)', _StubCodegen()))
